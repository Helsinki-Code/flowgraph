...
app.post("/v1/events", async (request, reply) => {
    try {
        ...
        if (sessionId) {
            ...
            await store.updateSessionTotals(sessionId, totalCostUsd, totalTokens, turnCount, loopDetected);
        }
 ...
 */
app.get("/v1/sessions/:id/flamegraph", async (request, reply) => {
    try {
        const { id } = request.params;
        const events = await store.getSessionEvents(id);
        if (events.length === 0) {
 ...
 */
app.get("/v1/cost/breakdown", async (request, reply) => {
    try {
        const workspaceId = request.query.workspace_id || "default";
        const groupBy = request.query.group_by || "tool";
...
app.get("/v1/alerts", async (request, reply) => {
    const alerts = await store.getAlerts(request.workspaceId);
    reply.send({ alerts });
});
/**
 * POST /v1/alerts
...
app.post("/v1/alerts", async (request, reply) => {
    const { name, condition, threshold, webhookUrl, email } = request.body;
    const id = crypto.randomUUID();
    await store.createAlert(id, request.workspaceId, name, condition, threshold, webhookUrl, email);
...
app.delete("/v1/alerts/:alertId", async (request, reply) => {
    await store.deleteAlert(request.params.alertId);
    reply.send({ ok: true });
});
/**
 * GET /v1/workspace
...
app.get("/v1/workspace", async (request, reply) => {
    const workspace = await store.getWorkspace(request.workspaceId);
